...
`CACHE_DIRECTORY` and `AI_KEY_SECRET`. It checks nothing about
...
8. **Environment** — copy `.env.local.example` to `shared/.env.local`, fill it in, and
   ...
   **`AI_KEY_SECRET` is one of those, and it is generated by hand here.** The Docker
   ...
   Put the output in `shared/.env.local` as `AI_KEY_SECRET=`. Activation refuses to
