...
(7) or `shared/.env.local` (8), and it reads that file's contents — `APP_ENV`,
`CACHE_DIRECTORY` and `INSTANCE_SECRET_KEY`. It checks nothing about
...
8. **Environment** — copy `.env.local.example` to `shared/.env.local`, fill it in, and
   ...
   **`INSTANCE_SECRET_KEY` is one of those, and it is generated by hand here.** The Docker
   deployment generates it in `scripts/prod-start.sh`; nothing on this host does, so it is
   your step:
   ...
   Put the output in `shared/.env.local` as `INSTANCE_SECRET_KEY=`. Activation refuses to
   continue while it is missing or still equal to the placeholder that `backend/.env`
   ships, because that placeholder is 48 characters long — `ApiKeyCipher` accepts it, every
   account's provider API key seals successfully, and the site works while every stored key
   is sealed under a secret anyone can read on GitHub.

   Keep the secret, in the password manager, beside `JWT_PASSPHRASE`. It is not rotatable
   in place: it is the master secret every stored API key is derived from, so changing it
   turns every one of them into ciphertext nobody can open, and each account is told only
   to enter its key again.
