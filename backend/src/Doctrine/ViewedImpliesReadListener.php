...
/**
 * Enforces the one invariant of the read model (#482): a viewed entry is always
 * read. "Viewed" means the user opened and read the article; "read" means it has
 * left the unread list. Every viewed entry must be read, so "Recently read" is a
 ...
 * read" UPDATE bypasses ORM events, but it only ever sets the read flag, so it
 * cannot violate the invariant and needs no coverage here.
 *
 * readAt takes the entry's own viewedAt when present, so the two timestamps
 * agree; only a viewed row with no viewedAt (which markViewed never produces)
 * falls back to the clock.
 */
#[AsDoctrineListener(event: Events::onFlush)]
final readonly class ViewedImpliesReadListener
{
    ...
    public function onFlush(OnFlushEventArgs $args): void
    {
        ...
        foreach ($scheduled as $entity) {
            if (!$entity instanceof EntryState || !$entity->isViewed() || $entity->isRead()) {
                continue;
            }

            $entity->markRead($entity->getViewedAt() ?? $this->clock->now());
