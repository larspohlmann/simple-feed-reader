 ...
 *
 * The two sources need different rules (#639). The article's own og:image
 * (`selectArticleHero`) is suppressed when the extracted body already *leads*
 * with an image, and when the body repeats that same photo lower down — matched
 * by CDN image identity, so an article that shows its lead photo again at
 * another size never stacks it twice (#505/#590/#608/#610/#619/#625). The feed
 * image (`selectFeedHero`) is only a fallback for an article with no picture of
 * its own, so it is suppressed whenever the body carries any image at all.
 *
...
final class HeroImageSelector
{
    ...
    /** @phpstan-assert-if-true !null $candidate */
     ...
     *     `/assets/…-tn-clean-100~384x216`, zdfheute.de entry 478680; and the
     *     WordPress `…/kulturbrauerei-berlin-1024x605.jpg` vs `…-1430x845.jpg`,
     *     looprituals.de entry 480179) — the trailing size token is dropped,
     *     leaving the path and the basename's stable part. This differs from
     *     zeit, where the whole basename is a variant crop word: zeit's `__`
     *     marks the whole basename for removal, so it is handled first and never
     *     reaches this narrower rule.
