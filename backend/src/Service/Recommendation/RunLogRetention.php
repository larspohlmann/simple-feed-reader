 ...
 *
 * It used to be one: the log was a live view of the current run, so a new run
 * wiped the last (#309). Investigations since have been comparative instead —
 * #396 and #399 each needed one run set beside another — and a one-run-wide
 * window leaves nothing to compare against, so each question waits for a second
 * failure. Since #638 the log is also the phase-timing history the ETA averages,
 * so the window bounds how many past runs that estimate learns from.
 *
