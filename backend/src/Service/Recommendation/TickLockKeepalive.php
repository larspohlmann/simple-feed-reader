 ...
 *
 * RecommendationRunAdvancer's per-user lock used to be sized for the longest legal
 * call a holder could make (three hours five minutes on the slow profile), because
 * nothing renewed it and a second process taking the lock mid-tick could double-bank
 * winners and double-bill provider spend. Twice a worker died mid-tick instead, and
 * that same TTL stranded the run for hours against a holder that no longer existed.
 *
 ...
 * around its tick and sizes the TTL against that silence; this class only refreshes.
 * The number that sizing must respect is NOT MINIMUM_INTERVAL_SECONDS — a beat fires
 * only from inside OpenAiCompatibleChatClient::completeMany(), once per chunk, so the
 * real ceiling is set by everything that produces no chunk:
 *
 ...
 * - Nothing beats between acquire() and the first request: candidate loading and
 *   prompt assembly run first.
 ...
 * re-acquired the lock: the double-bank is underway. beat() never throws it on —
 * throwing into the streaming loop is the failure this class avoids, and the tick is
 * mid-HTTP-call with nowhere safe to unwind. It records the loss instead, and
 * RecommendationTickCheckpoint reads it: the tick stops at its next checkpoint rather
...
final class TickLockKeepalive implements CompletionStreamHeartbeat
{
     ...
     * LockInterface exposes no such accessor — and exists only so a lost refresh
     * logs against the run it stranded, not an anonymous lock. The throttle resets
     * with the arming: a fresh tick must not inherit the previous tick's beat clock,
     * or the new lock goes unrefreshed for up to MINIMUM_INTERVAL_SECONDS. A recorded
     * loss resets too: one instance serves every tick a worker runs, and the tick
     * that lost its lock is over.
     */
