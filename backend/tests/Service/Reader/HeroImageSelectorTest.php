...
final class HeroImageSelectorTest extends TestCase
{
    ...
    /**
     * The article rule is about URLs, so the cases below state URLs. Dimensions
     * ride along untouched and have their own case at the end of this class.
     */
    private function selectArticleUrl(?string $candidateUrl, string $bodyHtml): ?string
    {
        $candidate = $candidateUrl === null ? null : new DeclaredImage($candidateUrl);

        return $this->selector->selectArticleHero($candidate, $bodyHtml)?->url;
    }

    // --- Article hero: lead-position rule (#520) --------------------------------

    public function testShowsArticleHeroWhenTheBodyImageIsADifferentPicture(): void
    {
        ...
        self::assertSame($hero, $this->selectArticleUrl($hero, $body));
    }

    public function testSuppressesArticleHeroWhenTheBodyLeadsWithADifferentPhoto(): void
    {
        ...
        self::assertNull($this->selectArticleUrl($hero, $body));
    }

    public function testSuppressesArticleHeroWhenALinkedImageLeadsTheBody(): void
    {
    ...
    public function testShowsArticleHeroWhenTextPrecedesAnInlineImageInTheFirstBlock(): void
    {
    ...
    public function testShowsArticleHeroWhenANonBreakingSpaceLeadsBeforeTheImage(): void
    {
        // A non-breaking space is visible text, not layout whitespace, so it
        // counts as a text lead and the hero stays.
        $hero = 'https://cdn.test/hero.jpg';
    ...
    public function testSuppressesArticleHeroWhenLayoutWhitespacePrecedesTheLeadingImage(): void
    {
    ...
    // --- Article hero: repeat rule (CDN image identity) -------------------------

    public function testSuppressesArticleHeroWhenTheBodyRepeatsTheHeroLowerDownUnderATextLead(): void
    {
        // The #505 dedup: the body leads with text, so the lead-position rule does
        // not fire, but it repeats the hero photo (a size variant) lower down, so a
    ...
    public function testSuppressesArticleHeroWhenTheHeroPhotoIsNotTheFirstBodyImage(): void
    {
    ...
    public function testSuppressesArticleHeroWhenTheBodyShowsTheSameImageUnderASizeVariantUrl(): void
    {
    ...
    public function testSuppressesArticleHeroWhenOnlyTheQueryStringDiffers(): void
    {
    ...
    {
        // The body leads with text, so only the repeat rule can suppress the hero
        // here: the two URLs match on identity alone, which they do only because
        // the identity is lower-cased on both sides.
    ...
    {
        // A pathless URL is its own identity, and that fallback is lower-cased too,
        // so a host that differs only in case is still the same picture. The body
        // leads with text, so only the repeat rule can fire.
        $hero = 'https://CDN.test';
    ...
    public function testShowsArticleHeroWhenTwoPathlessUrlsDifferOnlyInTheirHost(): void
    {
        // A lone slash names no photo, so trimming it makes each URL its own
        // identity and two different hosts stay distinct — the hero still leads.
        // Without the trim both paths collapse to "/" and wrongly match.
        $hero = 'https://a.test/';
    ...
    public function testShowsTheArticleHeroWhenItHasNoPathBasename(): void
    {
        // parse_url returns no path for a bare host; the hero keeps its whole form
        // as its identity, so it does not match the different body image. The body
        // leads with text, so the lead-position rule does not fire either, and the
        // hero still leads.
        $hero = 'https://cdn.test';
        $body = '<p>Intro.</p><img src="https://cdn.test/photo.jpg" alt="">';
    ...
    public function testSuppressesArticleHeroWhenAWordpressBasenameSizeSuffixRepeatsThePhoto(): void
    {
        // The looprituals.de entry 480179 regression (#653): a WordPress feed whose
        // extracted body leads with an "Anzeige" ad image (a text lead), then
        // repeats the og:image at another `-WIDTHxHEIGHT` basename size. The hero
        // is `kulturbrauerei-berlin-1024x605.jpg`, the body figure
        // `kulturbrauerei-berlin-1430x845.jpg`. Without the repeat rule the reader
        // stacks the same photo twice.
        $photo = 'https://looprituals.de/app/uploads/2026/08/kulturbrauerei-berlin';
        $hero = $photo . '-1024x605.jpg';
        $body = '<div><p><span>Anzeige</span></p><p><a href="https://ad.test">'
            . '<img src="https://looprituals.de/app/uploads/2026/08/2608_SonicSales.jpg" alt=""></a></p></div>'
            . '<figure><img src="' . $photo . '-1430x845.jpg" alt=""></figure>';

        self::assertNull($this->selectArticleUrl($hero, $body));
    }

    public function testSuppressesArticleHeroWhenACdnPutsThePhotoIdentityInThePathAndTheSizeInTheBasename(): void
    {
        // The zeit.de entry 480597 regression (#653): one photo lives in an
        // image-group directory and each size is a differently named basename, so
        // the hero and the body figure are the same picture under `wide__1300x731`
        // and `wide__660x371`. A "Quelle: dpa" byline leads the body, so only the
        // repeat rule can catch it; it must, or the reader stacks the photo twice.
        $imageGroup = 'https://img.zeit.de/news/2026-08/26/ehrenamt-image-group';
        $hero = $imageGroup . '/wide__1300x731';
    ...
    public function testSuppressesArticleHeroWhenTheSamePhotoUsesADifferentCropNameInTheBody(): void
    {
        ...
        // `wide__660x371`. The directory is the photo's identity; the crop word is
        // part of the size variant.
    ...
    public function testShowsArticleHeroWhenTheBodyPhotoBelongsToADifferentImageGroup(): void
    {
        // Distinct photos live in distinct image-group directories, so a
        // size-variant basename shared between them must not collapse the two into
    ...
    public function testSuppressesArticleHeroWhenACdnPutsTheSizeInItsOwnPathSegment(): void
    {
    ...
    public function testShowsArticleHeroWhenAPathSegmentSizeBelongsToADifferentPhoto(): void
    {
    ...
    public function testSuppressesArticleHeroWhenTheSizeIsABasenameSuffixOnAStablePhotoName(): void
    {
    ...
    public function testShowsArticleHeroWhenABasenameSuffixSizeBelongsToADifferentPhoto(): void
    {
    ...
    public function testSuppressesArticleHeroWhenTheSizeIsATildeSeparatedBasenameSuffix(): void
    {
        // The zdfheute.de entry 478680 case (#619): one photo lives at a stable
        // asset name and each size is a `~WIDTHxHEIGHT` suffix on it — the extracted
        // og:image hero is `…-tn-clean-100~1280x720` while the body repeats it as
        // `…-tn-clean-100~384x216`. A "Jetzt live" paragraph leads the body, so only
        // the repeat rule can catch it.
    ...
    public function testShowsArticleHeroWhenATildeSeparatedSizeBelongsToADifferentPhoto(): void
    {
    ...
    public function testSuppressesArticleHeroWhenTheSamePhotoSitsBehindDifferentCdnFetchTransforms(): void
    {
        // The natehagens.substack.com entry 1359963 case (#625): Substack serves
        // images through a Cloudinary-style fetch proxy that carries the real photo
        // — an encoded origin URL — as the last path segment, behind a transform
        // segment that differs per size. The extracted og:image hero and the body
        // figure are the same S3 object (`844466a9-…`) under the transforms
        // `w_1200,h_675,c_fill,…` and `w_1456,c_limit,…`. An intro paragraph leads
        // the body, so only the repeat rule can catch it.
    ...
    public function testShowsArticleHeroWhenAFetchProxyOriginBelongsToADifferentPhoto(): void
    {
    ...
    {
        // The proxy unwrap only triggers when the last path segment *is* an encoded
        // origin — i.e. it begins with the scheme. Two distinct photos whose file
        // names merely embed a URL-like substring further along keep their own path
        // identities and must not be collapsed into one, so the hero stays. The
        // body leads with text, so only the repeat rule can fire.
    ...
    // --- Feed hero: any body image suppresses it (#639) -------------------------
    ...
    {
        // The rule matches the element name `img` exactly, so an `<imgur-embed>` is
        // neither a leading image nor a body image.
        $hero = new DeclaredImage('https://cdn.test/feed.jpg');
    ...
    public function testAcceptsAnUppercaseScheme(): void
    {
        ...
        self::assertSame($hero, $this->selectArticleUrl($hero, '<p>Just words.</p>'));
    }

    public function testRejectsANonHttpHeroFromEitherSource(): void
    {
        // The scheme guard is anchored: a `data:` URL that merely embeds an http(s)
        // address later in its payload is still rejected.
        self::assertNull($this->selectArticleUrl('javascript:alert(1)', '<p>Body.</p>'));
        self::assertNull($this->selectArticleUrl('data:text/html,<a href="http://evil.test">x</a>', '<p>Body.</p>'));

        $data = new DeclaredImage('data:text/html,<a href="http://evil.test">x</a>');
        self::assertNull($this->selector->selectFeedHero($data, '<p>Body.</p>'));
    }

    public function testReturnsNullWhenThereIsNoHero(): void
    {
        self::assertNull($this->selectArticleUrl(null, '<p>Body.</p>'));
        self::assertNull($this->selector->selectFeedHero(null, '<p>Body.</p>'));
    }
    ...
    public function testKeepsAHeroWhenTheBodyIsUnparsable(): void
    {
        ...
        self::assertSame($hero, $this->selector->selectArticleHero($hero, "\0"));
        self::assertSame($hero, $this->selector->selectFeedHero($hero, "\0"));
    }

    public function testKeepsTheDeclaredDimensionsOfAnAcceptedArticleHero(): void
    {
        // The dimensions are the client's aspect-ratio reservation, so the selector
        // must hand back the candidate itself, not a rebuilt copy.
        $hero = new DeclaredImage('https://cdn.test/hero.jpg', 800, 450);

        $selected = $this->selector->selectArticleHero($hero, '<p>Just words.</p>');
