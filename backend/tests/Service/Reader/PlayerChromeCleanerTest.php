...
final class PlayerChromeCleanerTest extends TestCase
{
    ...
    private const string SILENT_TTS_WIDGET =
        ...
        . '<div><p>Listen to this article</p><p>Estimated 3 minutes</p></div>'
        . '<div class="toggletip ttsToggleTip"><p>The audio version of this article is '
        . 'generated by AI-based technology. Mispronunciations can occur.</p></div></div>';

    public function testRemovesASilentTextToSpeechWidgetWholeIconAndLabels(): void
    {
        $html = '<div>' . self::SILENT_TTS_WIDGET . '<p>' . self::PROSE . '</p></div>';

        $clean = $this->clean($html);

        self::assertStringNotContainsString('texttospeech.svg', $clean);
        self::assertStringNotContainsString('Listen to this article', $clean);
        self::assertStringNotContainsString('AI-based technology', $clean);
        self::assertStringContainsString('rubber band', $clean);
    }

    public function testKeepsAnArticleImageThatSitsBesideTheWidget(): void
    {
        // The widget goes; a lead figure that shares its parent stays.
        $html = '<div>' . self::SILENT_TTS_WIDGET
            . '<figure><img src="https://www.cbc.ca/news/photo.jpg" alt="An aerial view"></figure></div>';
    ...
    public function testKeepsANarrationContainerThatStillHoldsItsPlayer(): void
    {
        ...
        self::assertStringContainsString('<audio', $clean);
        self::assertStringContainsString('Listen to this article', $clean);
        self::assertStringContainsString('rubber band', $clean);
    }

    public function testLeavesAPlainContentImageThatNamesNoNarration(): void
    {
        $html = '<div><figure><img src="https://pub.test/hero-photo.jpg" alt="A photo">'
            . '</figure><p>' . self::PROSE . '</p></div>';

        $clean = $this->clean($html);

        self::assertStringContainsString('hero-photo.jpg', $clean);
        self::assertStringContainsString('rubber band', $clean);
    }
