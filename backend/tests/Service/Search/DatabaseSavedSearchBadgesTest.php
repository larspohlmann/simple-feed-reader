...
/**
 * DatabaseSavedSearchBadges: the LIKE badge scan behind the shared interface.
 * It must reproduce SavedSearchEntryRepository::unreadMatchIdsBySavedSearch
 * exactly, so SavedSearchBadgesWithFallback can hand either path's result to
 * the sidebar unchanged.
 */
final class DatabaseSavedSearchBadgesTest extends DbTestCase
{
    private User $user;
    ...
    public function testReturnsTheRepositoryUnreadMatchSet(): void
    {
        ...
        $idsBySearch = (new DatabaseSavedSearchBadges($repository))->unreadMatchIdsBySavedSearch(
            $this->user->getId() ?? 0,
            [new SavedSearchTerm(1, SearchTerms::fromInput('angular'))],
        );

        self::assertSame([1 => [$this->unreadId]], $idsBySearch);
    }
