...
| # | Decision | Why |
|---|---|---|
| D1 | Discovery reads the **raw source page**, in its own scanner | Discovery must not depend on `FetchedPageNormalizer`: that pass is tuned for readability scoring, it *removes* elements (share widgets, screen-reader-only nodes, and now the Substack player), and it is free to change again. It also strips every `<script>` from the raw string *before* parsing (`FetchedPageNormalizer.php:119`, pattern at `:65`) — a real hazard for a future host whose media lives only in a script, even though the captured hosts did not turn out to be such a case (ARD's renditions are in a `data-v` attribute, not a script; the sanitizer strips `data-*`, so the raw page is still where they survive). A normalizer-based scanner would silently find nothing for such a host while appearing to work for the others. |
