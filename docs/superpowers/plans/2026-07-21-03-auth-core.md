...
**Tech Stack:** Symfony 7.4 LTS, `symfony/security-bundle`, `lexik/jwt-authentication-bundle` ^3.2, `symfony/validator`, `symfony/serializer`, `symfony/mailer`, `symfony/rate-limiter`. No Twig (emails are plain strings). No ALTCHA library — the proof-of-work is implemented here.

> **Version note (added during execution):** this plan was written against Symfony 7.3. Task 1's audit found 7.3 is EOL with 22 unpatched advisories — including a firewall bypass in `security-http` that would undermine Task 7 — so Task 1b upgraded the whole project to 7.4 LTS before any auth code was written. All `7.3.*` constraints below now read `7.4.*`.
