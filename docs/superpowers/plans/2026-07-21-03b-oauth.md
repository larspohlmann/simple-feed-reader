...
```yaml
    # The flow tests must mint and inspect state and login codes, whose values
    # exist only inside a redirect URL. Same `autowire: true` caveat as the
    # entries above: naming a class here REPLACES the definition the App\
    # resource produced, and this file has no _defaults to inherit from, so
    # without it the service is built with no constructor arguments.
    ...
    # Flow-level tests swap the real provider for a fake via
    # `self::getContainer()->set(...)`, which only works on public services.
    App\Service\OAuth\GoogleOAuthProvider:
        autowire: true
        public: true

    App\Service\OAuth\AppleOAuthProvider:
        autowire: true
        public: true
