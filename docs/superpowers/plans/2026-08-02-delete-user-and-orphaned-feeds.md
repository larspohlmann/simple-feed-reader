...
- **Any assertion about a `Feed` after a reclaim MUST clear the identity map first.** `OrphanedFeedReclaimer` deletes through bulk DQL, which never updates the unit of work, and `KernelBrowser` does not reboot the kernel between requests. `EntityManager::find()` therefore returns the stale pre-delete object without touching the database. Call `$em->clear()` (or count with raw SQL) before **every** `assertNull`/`assertNotNull` on a feed — including the "the feed survives" cases. Skipping it there is worse than useless: a Task 2 review proved such a test still passed while the shared feed had genuinely been deleted from the database. Every feed assertion in this plan must be falsifiable; verify by temporarily breaking the guard and watching the test fail.
